    ...
    <link rel="apple-touch-icon-precomposed" href="${req.contextPath}/images/assets/ico/apple-touch-icon-57-precomposed.png">
  	<script type="text/javascript">
  		function solutionAdd(){
  			window.location.href="${req.contextPath}/manager/solution/toSolutionAdd";
  		}
  	</script>
  </head>
...
<div class="btn-toolbar">
    <button class="btn btn-primary" onclick="solutionAdd();"><i class="icon-plus"></i>新增</button>
...
<div class="well">
    <table class="table table-bordered table-hover">
      ...
      <tbody>
	      <c:forEach items="${solutionList}" var="solution" varStatus="status">
	      	 <tr>
	          <td>1</td>
	          <td>${solution.soluTitle}</td>
	          <td><fmt:formatDate value="${solution.createTime}" pattern="yyyy年MM月dd日  HH时mm分ss秒"/></td>
	          <td>${solution.isShow}</td>
	          <td>
	              <a href="${req.contextPath}/manager/solution/toSolutionUpdate?soluId=${solution.soluId}"><i class="icon-pencil">修改</i></a>
	          </td>
	        </tr>
	   	  </c:forEach>
      </tbody>
